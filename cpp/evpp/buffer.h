...
// @see https://github.com/chenshuo/muduo/blob/master/muduo/net/Buffer.h and
...
class Buffer {
 public:
  ...
  // c-style string text. It will not change the length of buffer.
  void ToText() {
    AppendInt8('\0');
    UnwriteBytes(1);
  }
...
#define evppbswap_64(x)                                                        \
 ...
 public:
  ...
  void Append(const Slice& str) { Write(str.data(), str.size()); }
 ...
 public:
  ...
  Slice ToSlice() const { return Slice(data(), length()); }
  ...
  // If there are fewer than n bytes in the buffer, Next returns the entire
  // buffer. The slice is only valid until the next call to a read or write
  // method.
 ...
 public:
  // data returns a pointer of length Buffer.length() holding the unread portion
  // of the buffer. The data is valid for use only until the next buffer
  // modification (that is, only until the next call to a method like Read,
  // Write, Reset, or Truncate). The data aliases the buffer content at least
  // until the next buffer modification, so immediate changes to the slice will
  // affect the result of future reads.
  ...
  // capacity returns the capacity of the buffer's underlying byte slice, that

...
bool ParseFromIPPort(const char* address, struct sockaddr_storage& ss) {
  ...
  if (rc == 0) {
    LOG_INFO << "ParseFromIPPort evutil_inet_pton (AF_INET '" << host.data()
             << "', ...) rc=0. " << host.data()
    ...
    if (serrno == 0) {
      LOG_INFO << "[" << host.data()
               << "] is not a IP address. Maybe it is a hostname.";
    } else {
      LOG_WARN << "ParseFromIPPort evutil_inet_pton (AF_INET, '" << host.data()
               << "', ...) failed : " << strerror(serrno);
    }
    return false;
  }
...
bool SplitHostPort(const char* address, std::string& host, int& port) {
  ...
  if (host[0] == '[') {
    if (*host.rbegin() != ']') {
      LOG_ERROR << "Address specified error <" << address
                << ">. '[' ']' is not pair.";
      return false;
    }
...
std::string ToIPPort(const struct sockaddr_storage* ss) {
  ...
  if (ss->ss_family == AF_INET) {
    struct sockaddr_in* addr4 =
        const_cast<struct sockaddr_in*>(sockaddr_in_cast(ss));
    char buf[INET_ADDRSTRLEN] = {};
    const char* addr = ::evutil_inet_ntop(ss->ss_family, &addr4->sin_addr, buf,
                                          INET_ADDRSTRLEN);
    ...
    struct sockaddr_in6* addr6 =
        const_cast<struct sockaddr_in6*>(sockaddr_in6_cast(ss));
    char buf[INET6_ADDRSTRLEN] = {};
    const char* addr = ::evutil_inet_ntop(ss->ss_family, &addr6->sin6_addr, buf,
                                          INET6_ADDRSTRLEN);
...
bool IsEqualIp(const struct sockaddr_storage& left,
               const struct sockaddr_storage& right) {
  if (left.ss_family != right.ss_family) {
    return false;
  }
  if (left.ss_family == AF_INET) {
    auto laddr4 = sockaddr_in_cast(&left);
    auto raddr4 = sockaddr_in_cast(&right);
    return ::memcmp(laddr4, raddr4, sizeof(struct sockaddr_in)) == 0;
  } else if (left.ss_family == AF_INET6) {
    auto laddr6 = sockaddr_in6_cast(&left);
    auto raddr6 = sockaddr_in6_cast(&right);
    return ::memcmp(laddr6, raddr6, sizeof(struct sockaddr_in6)) == 0;
  }
  return false;
  // TODO
  // return IsZeroAddress(&left) && IsZeroAddress(&right);
  // return ::memcmp(left, right, sizeof(struct sockaddr_storage)) == 0;
}

bool IsEqualPort(const struct sockaddr_storage& left,
                 const struct sockaddr_storage& right) {
  auto laddr4 = sock::sockaddr_in_cast(&left);
  auto raddr4 = sock::sockaddr_in_cast(&right);
  return laddr4->sin_port == raddr4->sin_port;
}

bool IsEqual(const struct sockaddr_storage& left,
             const struct sockaddr_storage& right) {
  return IsEqualIp(left, right) && IsEqualPort(left, right);
}
...
void SetKeepAlive(evpp_socket_t fd, bool on) {
  ...
  if (rc != 0) {
    int serrno = errno;
    LOG_ERROR << "setsockopt(SO_KEEPALIVE) failed, errno=" << serrno << " "
              << strerror(serrno);
  }
}
...
void SetReuseAddr(evpp_socket_t fd) {
  ...
  if (rc != 0) {
    int serrno = errno;
    LOG_ERROR << "setsockopt(SO_REUSEADDR) failed, errno=" << serrno << " "
              << strerror(serrno);
  }
}
...
void SetReusePort(evpp_socket_t fd) {
  ...
  if (rc != 0) {
    int serrno = errno;
    LOG_ERROR << "setsockopt(SO_REUSEPORT) failed, errno=" << serrno << " "
              << strerror(serrno);
  }
#endif
}
...
void SetTCPNoDelay(evpp_socket_t fd, bool on) {
  ...
  if (rc != 0) {
    int serrno = errno;
    LOG_ERROR << "setsockopt(TCP_NODELAY) failed, errno=" << serrno << " "
              << strerror(serrno);
  }
}

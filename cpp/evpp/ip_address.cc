...
static bool IPIsHelper(const IPAddress& ip, const in6_addr& tomatch,
                       int length);
...
bool IPAddress::IsNil() const { return IPIsUnspec(*this); }
...
in6_addr IPAddress::ipv6_address() const { return u_.ip6; }

in_addr IPAddress::ipv4_address() const { return u_.ip4; }
...
std::string IPAddress::ToString() const {
  ...
  if (!::evutil_inet_ntop(family_, src, buf, sizeof(buf))) {
    return std::string();
  }
...
std::string IPAddress::ToSensitiveString() const {
  switch (family_) {
    case AF_INET: {
      ...
      if (find_pos == std::string::npos) return std::string();
...
boost::optional<IPAddress> MakeIPAddress(const addrinfo& info) {
  ...
  if (info.ai_addr->sa_family == AF_INET) {
    ...
    sockaddr_in6* addr = reinterpret_cast<sockaddr_in6*>(info.ai_addr);
    return IPAddress(addr->sin6_addr);
  }
  return boost::none;
}
...
boost::optional<IPAddress> MakeIPAddress(const std::string& str) {
  IPAddress addr;
  if (::evutil_inet_pton(AF_INET, str.c_str(), &addr.u_.ip4) == 0) {
    if (::evutil_inet_pton(AF_INET6, str.c_str(), &addr.u_.ip6) == 0) {
      return boost::none;
    } else {
...
bool IPIsUnspec(const IPAddress& ip) { return ip.family() == AF_UNSPEC; }
...
IPAddress TruncateIP(const IPAddress& ip, int length) {
  ...
  if (ip.family() == AF_INET) {
    ...
    uint32_t host_order_ip = ::ntohl(ip.ipv4_address().s_addr);
    in_addr masked;
    masked.s_addr = ::htonl(host_order_ip & mask);
    ...
    for (int i = 0; i < 4; ++i) {
      if (i == position) {
        uint32_t host_order_inner = ::ntohl(v6_as_ints[i]);
        v6_as_ints[i] = ::htonl(host_order_inner & inner_mask);
      } else if (i > position) {
        v6_as_ints[i] = 0;
      }
    }
...
int CountIPMaskBits(const IPAddress& mask) {
  ...
  switch (mask.family()) {
    case AF_INET: {
      word_to_count = ::ntohl(mask.ipv4_address().s_addr);
      break;
    }
    case AF_INET6: {
      ...
      if (i < 4) {
        word_to_count = ::ntohl(v6_as_ints[i]);
      }
      bits = (i * 32);
      break;
    }
